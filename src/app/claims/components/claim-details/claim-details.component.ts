...
export class ClaimDetailsComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.claimsService.getClaims().subscribe(claims => {
            ...
            this.claimDetailsForm.get('serviceProvider').setValue(this.claimData.serviceProvider);
            this.claimDetailsForm.get('serviceProviderType').setValue(this.claimData.serviceType);
            this.claimDetailsForm.get('claimDescription').setValue(this.claimData.claimDescription);
            this.claimDetailsForm.get('risk').setValue(this.claimData.risk);
            this.claimDetailsForm.get('activity').setValue(this.claimData.activity);
        ...
        this.perilService.getPerils().subscribe(perils => {
            this.perilsList = perils.filter(x => x.claimId === this.claimData.claimId);
        })
    }
        ...
        this.storage.upload(name, this.selectedFile).snapshotChanges().pipe(
            finalize(() => {
                fileRef.getDownloadURL().subscribe((url) => {
                    ...
                    this.claimsService.updateClaimDoc(this.claimData.claimId, claim);
                })
            })
        ).subscribe();
    }
    ...
    intimateClaim(): void {
        this.route.navigateByUrl('/claims/claim-transactions');
    }
}
